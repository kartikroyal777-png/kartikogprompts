...
const PromptCard: React.FC<PromptCardProps> = ({ prompt, onLikeToggle }) => {
  ...
  return (
    <motion.div 
      ...
      whileHover={{ y: -5 }}
      onClick={handleClick}
      className="group relative aspect-[4/5] rounded-xl md:rounded-2xl overflow-hidden bg-gray-100 dark:bg-gray-900 shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer border border-gray-200 dark:border-gray-800"
    >
      ...
      <div className="absolute top-2 left-2 md:top-3 md:left-3 z-20 flex flex-col gap-1.5 items-start pointer-events-none">
        ...
        {/* Bundle Badge - Monochrome */}
        {prompt.images && prompt.images.length > 1 && (
          <div className="bg-gray-200/90 text-black dark:bg-gray-800/90 dark:text-white backdrop-blur-md text-[9px] md:text-[10px] font-bold px-1.5 py-0.5 md:px-2.5 md:py-1 rounded-full flex items-center gap-1 shadow-lg border border-black/10 dark:border-white/10">
      ...
      <div className="absolute inset-0 p-2 md:p-4 flex flex-col justify-between pointer-events-none">
          ...
          <div className="flex items-center gap-1">
            <motion.button 
              ...
              className={cn(
                ...
                isLiked ? "bg-white text-black" : "bg-black/40 text-white hover:bg-white/20"
              )}
              onClick={handleLike}
            >
        ...
        <div className="space-y-0.5 md:space-y-1 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
          ...
          <div className="flex flex-wrap gap-1">
            <span className="inline-block px-1.5 py-0.5 bg-white/90 text-black backdrop-blur-sm text-[8px] md:text-[10px] font-bold uppercase tracking-wider rounded-full shadow-sm">
              {prompt.category}
            </span>
            {prompt.categories && prompt.categories.length > 1 && prompt.categories[1] !== prompt.category && (
               <span className="inline-block px-1.5 py-0.5 bg-gray-200/90 text-black backdrop-blur-sm text-[8px] md:text-[10px] font-bold uppercase tracking-wider rounded-full shadow-sm">
                {prompt.categories[1]}
              </span>
            )}
             {prompt.categories && prompt.categories.length > 2 && (
               <span className="inline-block px-1.5 py-0.5 bg-black/40 backdrop-blur-sm text-white text-[8px] md:text-[10px] font-bold uppercase tracking-wider rounded-full border border-white/20">
