...
export function getImageUrl(path: string | null | undefined, fallbackText: string = 'No Image'): string {
  ...
  if (!path) {
    return `https://img-wrapper.vercel.app/image?url=https://img-wrapper.vercel.app/image?url=https://img-wrapper.vercel.app/image?url=https://placehold.co/600x800/1e293b/FFF?text=${encodeURIComponent(fallbackText)}`;
  }
